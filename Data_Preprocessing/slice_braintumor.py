...
def save_slices(IM_path, gt_path,
    ...
    assert len(set(map_(np.shape, [fl, T1w, T1gd, T2w, gt]))) == 1
    assert gt.dtype in [np.uint8], gt.dtype
    w, h, _, _ = flair.shape

    norm_fl = norm_arr(fl)  # We need to normalize the whole 3d img, not 2d slices
    norm_T1w = norm_arr(T1w)
    norm_T1gd = norm_arr(T1gd)
    norm_T2w = norm_arr(T2w)

    assert 0 == norm_fl.min() and norm_fl.max() == 1, (norm_fl.min(), norm_fl.max())
    assert 0 == norm_T1w.min() and norm_T1w.max() == 1, (norm_T1w.min(), norm_T1w.max())
    assert 0 == norm_T1gd.min() and norm_T1gd.max() == 1, (norm_T1gd.min(), norm_T1gd.max())
    assert 0 == norm_T2w.min() and norm_T2w.max() == 1, (norm_T2w.min(), norm_T2w.max())
    ...
    for j in range(fl.shape[-1]):
        ...
        gt_s = gt[:, :, j]
        slices = [fl_s, T1w_s, T1gd_s,T2w_s, gt_s]
        assert fl_s.shape == T1w_s.shape == T1gd_s.shape
        assert set(np.unique(gt_s)).issubset([0, 1, 2,3])
    ...
    """
    save_dir_fl: Path = Path(dest_dir, ct_dir)
