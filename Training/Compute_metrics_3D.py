...
fold_all_H1 = open(os.path.join(fold_path,'results-Dice-3D.csv'), "w")
fold_all_H1.write(f"file, metric \n")

fold_all_H2 = open(os.path.join(fold_path,'results-hauss-3D.csv'), "w")
...
for patient in patient_ids:
    ...
    Volume_3D_pred_1 = np.array([fix_it(np.load(p)[0])==1 for p in pred_paths])
    Volume_3D_pred_2 = np.array([fix_it(np.load(p)[0]) == 2 for p in pred_paths])
    '''
    ...
    Volume_3D_gt_1 = np.array([np.load(os.path.join(gt_dir, p))==1 for p in basenames])
    Volume_3D_gt_2 = np.array([np.load(os.path.join(gt_dir, p)) == 2 for p in basenames])


    #dice = dice_acc_3D(np.array(Volume_3D_pred),
    #                   np.array(Volume_3D_gt), 2)

    dice_1 = np.round(metrics.compute_dice_coefficient(Volume_3D_gt_1, Volume_3D_pred_1)*100,2)
    dice_2 = np.round(metrics.compute_dice_coefficient(Volume_3D_gt_2, Volume_3D_pred_2)*100,2)


    fold_all_H1.write(f"{patient}, {np.round((dice_1+ dice_2)/2.0, 2)} \n")

    sf_1 = metrics.compute_surface_distances(np.array(Volume_3D_pred_1),
                 np.array(Volume_3D_gt_1), [1,1, 1]) # 1mm^^ corresponding to  mm spacing
    sf_2 = metrics.compute_surface_distances(np.array(Volume_3D_pred_2),
                                             np.array(Volume_3D_gt_2), [1, 1,1])
    hd_1 = metrics.compute_robust_hausdorff(sf_1, 100)
    hd_2 =  metrics.compute_robust_hausdorff(sf_2, 100)

    fold_all_H2.write(f"{patient}, {np.round((hd_1+ hd_2)/2.0, 2)} \n")
    print(patient, np.round((dice_1+ dice_2)/2.0, 2), np.round((hd_1+ hd_2)/2.0, 2))
